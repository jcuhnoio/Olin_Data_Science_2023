...
``` r
df_samples %>% ggplot(aes(x = strength)) +
  geom_histogram()
```
...
- What is the mean strength of the material, approximately?
  - (Your response here) From the histogram, it seems to be near 40,000.
- To what extent can you tell what shape the distribution of the data
  has?
  - (Your response here) It has characteristics of a bell curve, where a
    central value tends to have more counts than other values.
- Assuming the scopus is the strength of an individual part made from
  ...
  - (Your response here) There are both real and induced variabilities,
    from the material properties themselves to manufacturing methods.
...
df_samples %>% 
  mutate(
    failed = g_break(strength) <= 0
  ) %>% 
  summarize(failure = mean(failed))
```
...
- Is this estimate of the probability of failure trustworthy? Why or why
  not?
  - (Your response here) For the samples gathered, it is trustworthy.
    For real life situations such as large scale manufacturing, I would
    not deem this as trustworthy, or at least satisfactory for such
    applications, as 25 measurements may not tell the entire story.
- Can you confidently conclude that `POF < 0.03`? Why or why not.
  - (Your response here) For the same reason mentioned above, I cannot
    confidently reach that conclusion.
...
``` r
## TODO:
df_fit <- fitdistr(x = df_samples$strength, densfun = "lognormal")
df_fit
```
...
- Compare this probability with your estimate from q2; is it more or
  less trustworthy?
  - (Your response here) The number of observations has not changed,
    only a distribution was applied to the data, therefore I would argue
    that both estimates are equally as trustworthy. However, as it is
    unsure the distribution being used reflects real-world behavior in
    the first place, I would go as far as to say that this could be
    misleading.
...
- Does the confidence interval above account for uncertainty arising
  ...
  - (Your response here) No, as the distribution is only fitted to the
    limited physical tests we had in the first place.
- What could you do to tighten up the confidence interval?
  - (Your response here) Increasing resolution of the Monte Carlo method
    could be attempted.
- Can you *confidently* conclude that `POF < 0.03`? Why or why not?
  - (Your response here) No, I am still reluctant to reach this
    conclusion with confidence merely derived from the limited physical
    tests.
...
- Does the confidence interval above account for uncertainty arising
  ...
  - (Your response here) No, this method does not use the Monte Carlo
    method.
- Does the confidence interval above account for uncertainty arising
  from *limited physical tests* (`df_samples`)? Why or why not?
  - (Your response here) Yes, it accounts for the limited physical
    tests, as it resamples the data, rather than applying a specific
    distribution to it.
- Can you confidently conclude that `POF < 0.03`? Why or why not?
  - (Your response here) Given the results of the bootstrap method, I
    cannot, as the upper bound is bigger than `POF < 0.03`.
