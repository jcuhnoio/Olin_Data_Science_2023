...
filename <- "./data/stang.csv" # csv has been altered to say nu, instead of mu
...
- How many aluminum alloys are in this dataset? How do you know?
  - There are at least four alloys of the same material with varying
    ...
    four to nine, since there are four thickness values recorded, with
    nine different observations.
- What angles were tested?
  - 0, 45, 90 degrees
- What thicknesses were tested?
  - 0.022, 0.032, 0.064, 0.081
- Does E values and nu values vary depending on the angle of the alloy?
...
``` r
df_stang_factor <- df_stang_long %>% 
  mutate(angle = as.factor(angle))

df_stang_factor %>%
  ggplot() +
  geom_boxplot(aes(x = angle, y = E))
```
...
- There does not seem to be any meaningful correlation between angle and
  E or nu from this observation. However, since there was a limited set
  of data to work with, having more measurements would be ideal to
...
- Does this graph support or contradict the claim above?
  - The graph contradicts the claim above, since for the claim to hold
