...
antibiotics_longer <- df_antibiotics %>% pivot_longer(col = !bacteria & !gram)

antibiotics_longer %>%
  ggplot(aes(y = value, x = bacteria, color = name, group = name)) +
  scale_y_continuous(trans='log10') +
  geom_point() +
  coord_flip() +
  theme(axis.text.y = element_text(face = ifelse(antibiotics_longer$gram == "negative", 'bold.italic', 'italic')))
