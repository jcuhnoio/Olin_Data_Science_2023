...
- To what extent can you tell what shape the distribution of the data has?
  - (Your response here) It has characteristics of a bell curve, where a central value tends to have more counts than other values.
- Assuming the scopus is the strength of an individual part made from this aluminum alloy, is the observed variability real or induced?
  - (Your response here) There are both real and induced variabilities, from the material properties themselves to manufacturing methods.
...
- Is this estimate of the probability of failure trustworthy? Why or why not?
  - (Your response here) For the samples gathered, it is trustworthy. For real life situations such as large scale manufacturing, I would not deem this as trustworthy, or at least satisfactory for such applications, as 25 measurements may not tell the entire story.
- Can you confidently conclude that `POF < 0.03`? Why or why not.
  - (Your response here) For the same reason mentioned above, I cannot confidently reach that conclusion.
...
- Assuming your scopus is the probability of failure `POF` defined above, does your estimate exhibit real variability, induced variability, or both?
  - (Your response here) Real variability is not induced in the estimates. Considering the stipulation that the dataset comes from ideal testing, by definition, POF should not have any real variability, but only induced variability.
- Does this confidence interval imply that `POF < 0.03`?
  - (Your response here) Yes.
- Compare this probability with your estimate from q2; is it more or less trustworthy?
  - (Your response here) The number of observations has not changed, only a distribution was applied to the data, therefore I would argue that both estimates are equally as trustworthy. However, as it is unsure the distribution being used reflects real-world behavior in the first place, I would go as far as to say that this could be misleading.
- Does the confidence interval above account for uncertainty arising from the *Monte Carlo approximation*? Why or why not?
  - (Your response here) Yes, uncertainty that arises from the Monte Carlo method has been accounted for. The confidence interval is derived from the mean and standard deviation of the Monte Carlo samples.
- Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
  - (Your response here) No, as the distribution is only fitted to the limited physical tests we had in the first place. 
- What could you do to tighten up the confidence interval?
  - (Your response here) Increasing resolution of the Monte Carlo method could be attempted.
- Can you *confidently* conclude that `POF < 0.03`? Why or why not?
  - (Your response here) No, I am still reluctant to reach this conclusion with confidence merely derived from the limited physical tests.
...
- How does this estimate compare with your Monte Carlo estimate above?
  - (Your response here) It returns a value within the confidence interval.
- Does this estimate have any uncertainty due to *Monte Carlo approximation*? Why or why not?
  - (Your response here) No, the monte carlo method is not being used at all here.
...
- Does this estimate have any uncertainty due to *limited physical tests*? Why or why not?
  - (Your response here) This remains, as the number of physical tests remain the same.
- With the scopus as the `POF`, would uncertainty due to *limited physical tests* be induced or real?
  - (Your response here) This would be an induced uncertainty. The method of measurement itself, which is intrinsically an induced uncertainty, includes the number of physical tests done.
...
- Does the confidence interval above account for uncertainty arising from *Monte Carlo approximation* of the POF? Why or why not?
  - (Your response here) No, this method does not use the Monte Carlo method.
- Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
  - (Your response here) Yes, it accounts for the limited physical tests, as it resamples the data, rather than applying a specific distribution to it.
- Can you confidently conclude that `POF < 0.03`? Why or why not?
  - (Your response here) Given the results of the bootstrap method, I cannot, as the upper bound is bigger than `POF < 0.03`.
