...
There tends to be a positive correlation between price and carat.
...
diamonds %>% 
  ...
  geom_point(aes(color = factor(cut)), alpha = 0.4)
```
...
- (Write your observations here!) The cut is a deciding factor for the
  price if the carat is same. Better cuts are usually more expensive.
